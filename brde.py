...
class Dialog_brde(QtGui.QDialog):
    ...
    def __solt_init(self):
        self.connect(self.__ui.brde_open, QtCore.SIGNAL("clicked()"), self.__click_open)
        self.connect(self.__ui.brde_close, QtCore.SIGNAL("clicked()"), self.__click_close)
        self.connect(self.__ui.brde_clean, QtCore.SIGNAL("clicked()"), self.__click_clean)
        self.connect(self.__ui.brde_start, QtCore.SIGNAL("clicked()"), self.__click_start)
        self.connect(self.__ui.brde_end, QtCore.SIGNAL("clicked()"), self.__click_end)

        self.connect(self.__ui.brde_zbconfirm, QtCore.SIGNAL("clicked()"), self.__click_zbconfirm)
        self.connect(self.__ui.brde_zbinfo, QtCore.SIGNAL("clicked()"), self.__click_zbinfo)

    def __scancom(self, com_list):
        self.__ui.brde_serial.clear()
        self.__ui.brde_serial.addItems(com_list)
    ...
    def __click_open(self):
        ...
        if self.__ui.brde_comboBox.currentText() == '' or str(self.__ui.brde_serial.currentText()) == '':
            QtGui.QMessageBox.warning(self, u"错误" ,u"串口或波特率错误，请检查")
            return
        self.__serial_config(serial = self.ser, port = str(self.__ui.brde_serial.currentText()),
                             baudrate=int(self.__ui.brde_comboBox.currentText()))
        try:
            self.ser.open()
            ...
            self.__ui_disable(self.__ui.brde_open)

        except:
            QtGui.QMessageBox.warning(self, u"错误", u"串口被占用或异常，无法打开")

    def __click_clean(self):
        self.__ui.brde_display.clear()

    def __click_close(self):
        self.__ui_enable(self.__ui.brde_open)
        #结束全部线程
    ...
    def return_ui_display(self):
        return self.__ui.brde_display

    def __diaplay(self, temstr):
        self.__ui.brde_display.append(unicode(temstr))
