...
class Dialog_brtl(QtGui.QDialog):
    ...
    def __solt_init(self):
        self.connect(self.__ui.brtl_open, QtCore.SIGNAL("clicked()"), self.__click_open)
        self.connect(self.__ui.brtl_close, QtCore.SIGNAL("clicked()"), self.__click_close)
        self.connect(self.__ui.brtl_clean, QtCore.SIGNAL("clicked()"), self.__click_clean)
        self.connect(self.__ui.brtl_start, QtCore.SIGNAL("clicked()"), self.__click_start)
        self.connect(self.__ui.brtl_end, QtCore.SIGNAL("clicked()"), self.__click_end)

        self.connect(self.__ui.brtl_zbconfirm, QtCore.SIGNAL("clicked()"), self.__click_zbconfirm)
        self.connect(self.__ui.brtl_zbinfo, QtCore.SIGNAL("clicked()"), self.__click_zbinfo)

        #################################################################################
        self.connect(self.__ui.brtl_slinked, QtCore.SIGNAL("clicked()"), self.__click_slinked)
        self.connect(self.__ui.brtl_unslinked, QtCore.SIGNAL("clicked()"), self.__click_unslinked)
        self.connect(self.__ui.brtl_stime, QtCore.SIGNAL("clicked()"), self.__click_stime)
        self.connect(self.__ui.brtl_unstime, QtCore.SIGNAL("clicked()"), self.__click_unstime)
        self.connect(self.__ui.brtl_info, QtCore.SIGNAL("clicked()"), self.__click_info)
        self.connect(self.__ui.brtl_schipdir, QtCore.SIGNAL("clicked()"), self.__click_schipdir)
        self.connect(self.__ui.brtl_unschipdir, QtCore.SIGNAL("clicked()"), self.__click_unschipdir)
        self.connect(self.__ui.brtl_checkvhf, QtCore.SIGNAL("clicked()"), self.__click_checkvhf)
        self.connect(self.__ui.brtl_openvhf, QtCore.SIGNAL("clicked()"), self.__click_openvhf)
        self.connect(self.__ui.brtl_closevhf, QtCore.SIGNAL("clicked()"), self.__click_closevhf)
        self.connect(self.__ui.brtl_chechzb, QtCore.SIGNAL("clicked()"), self.__click_chechzb)
        self.connect(self.__ui.brtl_openzb, QtCore.SIGNAL("clicked()"), self.__click_openzb)
        self.connect(self.__ui.brtl_closezb, QtCore.SIGNAL("clicked()"), self.__click_closezb)
        self.connect(self.__ui.brtl_confirm, QtCore.SIGNAL("clicked()"), self.__click_confirm)
        #################################################################################

    def __scancom(self, com_list):
        self.__ui.brtl_serial.clear()
        self.__ui.brtl_serial.addItems(com_list)
    ...
    def __click_open(self):
        ...
        if self.__ui.brtl_comboBox.currentText() == '' or str(self.__ui.brtl_serial.currentText()) == '':
            QtGui.QMessageBox.warning(self, u"错误" ,u"串口或波特率错误，请检查")
            return
        self.__serial_config(serial = self.ser, port = str(self.__ui.brtl_serial.currentText()),
                             baudrate=int(self.__ui.brtl_comboBox.currentText()))
        try:
            self.ser.open()
            ...
            self.__ui_disable(self.__ui.brtl_open)

        except:
            QtGui.QMessageBox.warning(self, u"错误", u"串口被占用或异常，无法打开")

    def __click_clean(self):
        self.__ui.brtl_display.clear()

    def __click_close(self):
        self.__ui_enable(self.__ui.brtl_open)
        #结束全部线程
    ...
    def __click_confirm(self):
        self.__temid = self.__ui.brtl_id.text()
        self.__temid = "terconfig terid " + self.__temid + "\r\n"
        self.sin2writedata.emit(self.__temid)

        self.__temcdirmin = self.__ui.brtl_dirmin.text()
        self.__temcdirmin = "terconfig dirmin " + self.__temcdirmin + "\r\n"
        self.sin2writedata.emit(self.__temcdirmin)

        self.__temcdirmax = self.__ui.brtl_dirmax.text()
        self.__temcdirmax = "terconfig dirmax " + self.__temcdirmax + "\r\n"
        self.sin2writedata.emit(self.__temcdirmax)

        self.__temlat = self.__ui.brtl_lat.text()
        self.__temlat = "terconfig loclat " + self.__temlat + "\r\n"
        self.sin2writedata.emit(self.__temlat)

        self.__temlon = self.__ui.brtl_lon.text()
        self.__temlon = "terconfig loclon " + self.__temlon + "\r\n"
        self.sin2writedata.emit(self.__temlon)

        self.__llat = self.__ui.brtl_leftpierlat.text()
        self.__llat = "terconfig bllat " + self.__llat + "\r\n"
        self.sin2writedata.emit(self.__llat)

        self.__clat = self.__ui.brtl_centerpierlat.text()
        self.__clat = "terconfig bclat " + self.__clat + "\r\n"
        self.sin2writedata.emit(self.__clat)

        self.__rlat = self.__ui.brtl_rightpierlat.text()
        self.__rlat = "terconfig brlat " + self.__rlat + "\r\n"
        self.sin2writedata.emit(self.__rlat)

        self.__llon = self.__ui.brtl_leftpierlon.text()
        self.__llon = "terconfig bllon " + self.__llon + "\r\n"
        self.sin2writedata.emit(self.__llon)

        self.__clon = self.__ui.brtl_centerpierlon.text()
        self.__clon = "terconfig bclon " + self.__clon + "\r\n"
        self.sin2writedata.emit(self.__clon)

        self.__rlon = self.__ui.brtl_rightpierlon.text()
        self.__rlon = "terconfig brlon " + self.__rlon + "\r\n"
        self.sin2writedata.emit(self.__rlon)

        self.__water = self.__ui.brtl_water.text()
        self.__water = "terconfig wpatch " + self.__water + "\r\n"
        self.sin2writedata.emit(self.__water)

        self.__brname = self.__ui.brtl_brname.text()
    ...
    def return_ui_display(self):
        return self.__ui.brtl_display

    def __diaplay(self, temstr):
        self.__ui.brtl_display.append(temstr)
        self.__ui.brtl_display.currentFont()
